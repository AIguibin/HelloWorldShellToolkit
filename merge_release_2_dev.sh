...
for PROJECT in "${PROJECTS[@]}"; do
    ...
    # ==================== 第二阶段：pre_prod_release -> dev_alpha_deploy ====================
    ...
    # 合并 pre_prod_release 到 dev_alpha_deploy
    log "合并 pre_prod_release 到 dev_alpha_deploy..."
    if ! git merge pre_prod_release --no-commit >> "$APP_LOG" 2>&1; then
        error_log "${PROJECT}: pre_prod_release→dev_alpha_deploy 合并冲突"
        git merge --abort >> "$APP_LOG" 2>&1
        continue
    fi
	
    git commit -m "自动合并: pre_prod_release → dev_alpha_deploy  [$(date + '%Y-%m-%d %H:%M')]" >> "$APP_LOG" 2>&1
...
# ==================== 第三阶段：信贷天元前端分支公共文件夹app/export-template合并开始 ====================
...
git remote set-url origin http://10.12.168.107/ecms/ecms-web.git && git checkout pre_prod_release && git fetch --all && git reset --hard origin/pre_prod_release && git pull
...
# 第二步：从pre_prod_release合并到dev_alpha_deploy
if merge_folder "pre_prod_release" "dev_alpha_deploy"; then
	echo "所有合并操作完成"
else
	echo "第二步合并失败"
fi
...
git remote set-url origin http://10.12.168.107/ecms/ecms-web.git && git checkout pre_prod_release && git fetch --all && git reset --hard origin/pre_prod_release && git pull && git remote -v && git remote set-url origin http://10.12.170.176/ecms/ecms-web.git  && echo "新仓库" && git remote -v && git push  origin pre_prod_release --force && \
