...
MAVEN_SETTINGS="/d/Maven/config/ncms-admin-test-settings.xml"                  # Maven settings.xml 文件路径（可设置为绝对路径）
# 示例: MAVEN_SETTINGS="/opt/maven/conf/settings.xml"

# Git 配置
DEFAULT_BRANCH="SIT"             # 默认分支名称
...
deploy_single_project() {
    ...
    # 1. Git 操作（如果不跳过）
    if [ "$GIT_SKIP" != true ] && [ -n "$TARGET_BRANCH" ]; then
        if ! git_operations "$project_path" "$TARGET_BRANCH"; then
    ...
    if [ "$SKIP_TESTS" = true ]; then
        maven_cmd="$maven_cmd -DskipTests -Dmaven.test.skip=true -DskipITs -DaltDeploymentRepository=nexus-snapshots::default::http://192.15.3.99:10390/repository/maven-credit-snapshots/"
    fi
...
main() {
    ...
    if [ "$QUIET" = false ]; then
        ...
        if [ -n "$TARGET_BRANCH" ]; then
            ...
            echo -e "  Git 操作: $([ "$GIT_SKIP" = true ] && echo "跳过" || echo "执行")"
