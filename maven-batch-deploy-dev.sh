...
init_script() {
	# 进入文件夹
	cd /e/DeploySpace/dev || {
			echo "无法进入目录: /e/DeploySpace/dev"
			continue
	}
    # 创建日志目录
    mkdir -p "$LOG_DIR"
...
get_all_projects() {
    ...
    log_message "STEP" "正在扫描目录: $(realpath "$base_dir")" >&2
    ...
    while IFS= read -r dir; do
        if [ -f "$dir/pom.xml" ]; then
            projects+=("$(realpath "$dir")")
            log_verbose "发现项目: $(basename "$dir")" >&2
        fi
    done < <(find "$base_dir" -maxdepth 1 -mindepth 1 -type d 2>/dev/null)
    
    # 如果没有找到，再查找二级目录
    if [ ${#projects[@]} -eq 0 ]; then
        log_verbose "一级目录未找到项目，尝试查找二级目录..." >&2
        while IFS= read -r dir; do
            if [ -f "$dir/pom.xml" ]; then
                projects+=("$(realpath "$dir")")
                log_verbose "发现项目: $(basename "$dir")" >&2
    ...
    for project in "${projects[@]}"; do
        ...
        for exclude in "${exclude_array[@]}"; do
            ...
            if [[ -n "$exclude" ]] && [[ "$project" == *"$exclude"* ]]; then
                log_verbose "排除目录: $project (匹配模式: $exclude)" >&2
                include=false
                break
            fi
        done
...
show_help() {
    ...
    echo -e "  ${YELLOW}-i, --interactive${NC}         启用交互模式 (部署前需要确认)"
    ...
    echo -e "  $0 --modules"
    echo ""
    echo -e "  ${GRAY}# 部署单个项目（自动执行，无需确认）${NC}"
    echo -e "  $0 ncms-base-auth"
    echo ""
    echo -e "  ${GRAY}# 部署所有项目，需要确认${NC}"
    echo -e "  $0 -i"
    echo -e "${CYAN}══════════════════════════════════════════════════════════════${NC}"
}
...
main() {
    ...
    if [ "$PARALLEL_DEPLOY" = true ] && [ $TOTAL_PROJECTS -gt 1 ]; then
        ...
        for project_path in "${projects_to_deploy[@]}"; do
            # 确保即使失败也继续执行
            if ! deploy_single_project "$project_path"; then
                log_message "ERROR" "项目部署失败，继续执行下一个项目"
            fi
