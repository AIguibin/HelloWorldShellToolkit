...
git remote set-url origin http://10.12.168.107/ecms/ecms-web.git && git checkout test_alpha_deploy && test_alpha_deploy&& git remote -v && git remote set-url origin http://10.12.170.176/ecms/ecms-web.git  && echo "新仓库" && git remote -v && git push  origin test_alpha_deploy --force
